...
\title{Building a hardware accelerator for DNA sequence alignment  using an MPSoC}
\subtitle{acceleration of the Smith-Waterman algorithm for reference genome mapping with a Multi-Processor System on Chip?}
% \author{naam student}
\forenameA{Robin}
\surnameA{Nollet}
...
\promotorA[Supervisors]{Ing. Václav \v{S}imek}
...
\input{H1_Inleiding}
\input{H2_MBbackground}
\input{H3_Methods}
\input{H4_ELbackground}
\input{H4_InitialDifficulties}
\input{H5_RMaccelerated}
\input{H6_implementation}
%\input{H7_results}
\input{H8_conclusions}
